...
namespace TestowanieOprogramowania
{
    public partial class StanMagazynu : Form
    {
        ...
        private void buttonDodajProdukt_Click(object sender, EventArgs e)
        {
            ...
            {
                using (var formDodaj = new FormDodajProdukt(productService, userService))
                {
        ...
        */
        ...
        {
            string connectionString = PolaczenieBazyDanych.StringPolaczeniowy();
            ProductHistoryService productHistoryService = new ProductHistoryService(connectionString);
            if (ZarzadzanieVoidami.PrzegladHistoriiUzupelniania())
            {
                FormHUSM formHUSM = new FormHUSM(productHistoryService);
                formHUSM.ShowDialog();
            }
            else
            {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                FormDodajIlosc formDodajIlosc = new FormDodajIlosc(nazwaTowaru, dostawca, opis, jednostkaMiary, dodajIloscProduktu, new UserService(PolaczenieBazyDanych.StringPolaczeniowy()));
